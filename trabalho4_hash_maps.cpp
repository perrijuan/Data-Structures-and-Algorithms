#include <iostream>
#include <vector>
//usei a bibloteca para o mapeamento de valores removidos e assim evitar a insercao no vetor para imprmir a saida dos valores da forma que sao inseridos 
...
public:
    ...
    void listarChaves() {
        // Exibe os valores na ordem inversa, ignorando os removidos
        for (auto it = ordemChegada.rbegin(); it != ordemChegada.rend(); ++it) {
            if (removidos.find(*it) == removidos.end()) {
                cout << *it << endl;
            }
        }
    }
